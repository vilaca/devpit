...
The per-provider call-set analysis required by Design_Decisions §8. Maps
each attention bucket (§4) to exact API calls, defines the merge-gate
field mapping, sets token guidance, and budgets a poll cycle (§9).
...
1. **GitHub notifications require a classic PAT.** The Notifications API
   ...
   §8 "notifications as change-signal" tier is **optional** on GitHub and
   the plugin must work without it (search-based polling covers all
   buckets). This is a capability declaration (§10), driven by token type.
2. **GitLab's public REST API has no ETag/304 support.** Conditional
   requests (§9) are a GitHub-only optimization. GitLab change detection
...
`/user` at all — these hit the §7 manual-fallback path. Store the
...
none; it can't mark todos done, which is fine — read-only §11).
...
(GraphQL `mergeRequestInteraction.reviewState` could batch this —
optimization, not v0.1.)
...
states need ≥ 16.11 (default-on since 17.2). The plugin should degrade
per §10 on older self-hosted instances **[verify at implementation:
...
- 429 handling: honor `Retry-After`; also read `RateLimit-Remaining` /
  `RateLimit-Reset` headers for the §16 sync log's `rate_remaining`.
...
6. Minimum supported GitLab version and the §10 degradation matrix for
   older instances.
...
- §8 (Discovery): on GitHub the notifications feed is conditional on a
  classic PAT; search polling is the baseline.
- §9 (Synchronization): "conditional ETag requests" applies to GitHub
  only; GitLab uses `updated_after` watermarks.
- Provider_Capability_Matrix.md: seed from the capability table above.
