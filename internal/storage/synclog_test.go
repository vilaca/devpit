...
func TestLatestOutcomesPerOperation(t *testing.T) {
	...
	entries := []SyncLogEntry{
		// fast_poll: three rows; latest is "degraded"
		{Ts: base, ConnectionID: "c1", Operation: "fast_poll", Outcome: "ok"},
		{Ts: base.Add(time.Minute), ConnectionID: "c1", Operation: "fast_poll", Outcome: "ok"},
		{Ts: base.Add(2 * time.Minute), ConnectionID: "c1", Operation: "fast_poll", Outcome: "degraded"},
		...
		{Ts: base, ConnectionID: "c2", Operation: "fast_poll", Outcome: "ok"},
	}
	...
	if byOp["fast_poll"] != "degraded" {
		t.Errorf("fast_poll outcome = %q, want degraded (latest row wins)", byOp["fast_poll"])
	}
